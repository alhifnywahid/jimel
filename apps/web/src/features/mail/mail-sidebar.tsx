  ...
  return (
    <Sidebar {...props}>
      <SidebarHeader>
        ...
        {/* Active-address panel - hidden when the sidebar is collapsed to icons. */}
...
/** Placeholder list while the inbox is loading. */
function InboxSkeleton() {
  return (
    <div className="flex flex-col gap-1 px-2 py-1 group-data-[collapsible=icon]:hidden">
      {Array.from({ length: 5 }).map((_, i) => (
        // biome-ignore lint/suspicious/noArrayIndexKey: static placeholder, no id
...
/** Empty state: the active address has not received any email yet. */
function EmptyInbox() {
  return (
    <div className="px-2 py-6 text-center text-muted-foreground text-xs group-data-[collapsible=icon]:hidden">
      No emails yet. Emails sent to this address will show up here.
    </div>
  );
}
