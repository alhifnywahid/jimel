...
export function MailView({ mail, onClose }: MailDisplayProps) {
      ...
      <div className="grid h-full place-items-center">
        <Empty className="border-none">
          <EmptyHeader>
            ...
            <EmptyTitle>Belum ada email dipilih</EmptyTitle>
            <EmptyDescription>
              Pilih salah satu email dari daftar untuk melihat isinya di sini.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      </div>
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      {/* Isi email - terpusat, lebar baca nyaman */}
      <div className="scrollbar-none min-h-0 flex-1 overflow-y-auto">
        <article className="mx-auto w-full max-w-2xl px-1 py-6">
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              <div className="flex items-center gap-1 text-muted-foreground text-xs">
                <span className="truncate">kepada {mail.to.map((r) => r.name).join(", ")}</span>
                <Popover>
                  <PopoverTrigger asChild>
                    <Button
                      ...
                      aria-label="Lihat detail pengirim"
                    >
                      <ChevronDown />
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent align="start" className="w-96 p-0 text-xs">
                    <dl className="grid grid-cols-[6rem_1fr] gap-x-3 gap-y-1.5 p-3">
                      <DetailRow label="dari" value={`${mail.from.name} <${mail.from.email}>`} />
                      {mail.replyTo ? <DetailRow label="balas ke" value={mail.replyTo} /> : null}
                      <DetailRow label="kepada" value={mail.to.map((r) => r.email).join(", ")} />
                      <DetailRow
                        label="tanggal"
                        value={format(new Date(mail.receivedAt), "d MMM yyyy, HH.mm")}
                      />
                      <DetailRow label="subjek" value={mail.subject} />
                      {mail.mailedBy ? (
                        <DetailRow label="dikirim oleh" value={mail.mailedBy} />
                      ) : null}
                      {mail.signedBy ? (
                        <DetailRow label="ditandatangani oleh" value={mail.signedBy} />
                      ) : null}
          ...
          {mail.body !== undefined ? (
            ...
            <div className="py-6 text-muted-foreground text-sm">Isi email tidak tersedia.</div>
          )}
        </article>
      </div>
    </div>
